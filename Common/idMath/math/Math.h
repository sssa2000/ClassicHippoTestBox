...
#define DEG2RAD(a)				( (a) * H3DMath::M_DEG2RAD )
#define RAD2DEG(a)				( (a) * H3DMath::M_RAD2DEG )

#define SEC2MS(t)				( H3DMath::FtoiFast( (t) * H3DMath::M_SEC2MS ) )
#define MS2SEC(t)				( (t) * H3DMath::M_MS2SEC )

#define	ANGLE2SHORT(x)			( H3DMath::FtoiFast( (x) * 65536.0f / 360.0f ) & 65535 )
#define	SHORT2ANGLE(x)			( (x) * ( 360.0f / 65536.0f ) )

#define	ANGLE2BYTE(x)			( H3DMath::FtoiFast( (x) * 256.0f / 360.0f ) & 255 )
...
public:
	...
	static bool					IsPowerOfTwo( unsigned int x );		// returns true if x is a power of 2
...
ID_INLINE float H3DMath::RSqrt( float x ) {
...
ID_INLINE float H3DMath::InvSqrt16( float x ) {
...
ID_INLINE float H3DMath::InvSqrt( float x ) {
...
ID_INLINE double H3DMath::InvSqrt64( float x ) {
	dword a = ((union _flint*)(&x))->i;
	union _flint seed;
...
ID_INLINE float H3DMath::Sqrt16( float x ) {
	return x * InvSqrt16( x );
}

ID_INLINE float H3DMath::Sqrt( float x ) {
	return x * InvSqrt( x );
}

ID_INLINE double H3DMath::Sqrt64( float x ) {
	return x * InvSqrt64( x );
}

ID_INLINE float H3DMath::Sin( float a ) {
	return sinf( a );
}

ID_INLINE float H3DMath::Sin16( float a ) {
	float s;
...
ID_INLINE double H3DMath::Sin64( float a ) {
	return sin( a );
}

ID_INLINE float H3DMath::Cos( float a ) {
	return cosf( a );
}

ID_INLINE float H3DMath::Cos16( float a ) {
	float s, d;
...
ID_INLINE double H3DMath::Cos64( float a ) {
	return cos( a );
}

ID_INLINE void H3DMath::SinCos( float a, float &s, float &c ) {
#ifdef _WIN32
	_asm {
		fld		a
		fsincos
...
ID_INLINE void H3DMath::SinCos16( float a, float &s, float &c ) {
	float t, d;
...
ID_INLINE void H3DMath::SinCos64( float a, double &s, double &c ) {
#ifdef _WIN32
	_asm {
		fld		a
		fsincos
...
ID_INLINE float H3DMath::Tan( float a ) {
	return tanf( a );
}

ID_INLINE float H3DMath::Tan16( float a ) {
	float s;
	bool reciprocal;
...
ID_INLINE double H3DMath::Tan64( float a ) {
	return tan( a );
}

ID_INLINE float H3DMath::ASin( float a ) {
	if ( a <= -1.0f ) {
		return -HALF_PI;
	}
	if ( a >= 1.0f ) {
...
ID_INLINE float H3DMath::ASin16( float a ) {
...
ID_INLINE double H3DMath::ASin64( float a ) {
	if ( a <= -1.0f ) {
		return -HALF_PI;
	}
	if ( a >= 1.0f ) {
...
ID_INLINE float H3DMath::ACos( float a ) {
	if ( a <= -1.0f ) {
		return PI;
	}
	if ( a >= 1.0f ) {
...
ID_INLINE float H3DMath::ACos16( float a ) {
	if ( FLOATSIGNBITSET( a ) ) {
		if ( a <= -1.0f ) {
			return PI;
		}
...
ID_INLINE double H3DMath::ACos64( float a ) {
	if ( a <= -1.0f ) {
		return PI;
	}
	if ( a >= 1.0f ) {
...
ID_INLINE float H3DMath::ATan( float a ) {
	return atanf( a );
}

ID_INLINE float H3DMath::ATan16( float a ) {
	float s;
...
ID_INLINE double H3DMath::ATan64( float a ) {
	return atan( a );
}

ID_INLINE float H3DMath::ATan( float y, float x ) {
	return atan2f( y, x );
}

ID_INLINE float H3DMath::ATan16( float y, float x ) {
	float a, s;
...
ID_INLINE double H3DMath::ATan64( float y, float x ) {
	return atan2( y, x );
}

ID_INLINE float H3DMath::Pow( float x, float y ) {
	return powf( x, y );
}

ID_INLINE float H3DMath::Pow16( float x, float y ) {
	return Exp16( y * Log16( x ) );
}

ID_INLINE double H3DMath::Pow64( float x, float y ) {
	return pow( x, y );
}

ID_INLINE float H3DMath::Exp( float f ) {
	return expf( f );
}

ID_INLINE float H3DMath::Exp16( float f ) {
...
ID_INLINE double H3DMath::Exp64( float f ) {
	return exp( f );
}

ID_INLINE float H3DMath::Log( float f ) {
	return logf( f );
}

ID_INLINE float H3DMath::Log16( float f ) {
	int i, exponent;
	float y, y2;
...
ID_INLINE double H3DMath::Log64( float f ) {
	return log( f );
}

ID_INLINE int H3DMath::IPow( int x, int y ) {
	int r; for( r = x; y > 1; y-- ) { r *= x; } return r;
}

ID_INLINE int H3DMath::ILog2( float f ) {
	return ( ( (*reinterpret_cast<int *>(&f)) >> IEEE_FLT_MANTISSA_BITS ) & ( ( 1 << IEEE_FLT_EXPONENT_BITS ) - 1 ) ) - IEEE_FLT_EXPONENT_BIAS;
}

ID_INLINE int H3DMath::ILog2( int i ) {
	return ILog2( (float)i );
}

ID_INLINE int H3DMath::BitsForFloat( float f ) {
	return ILog2( f ) + 1;
}

ID_INLINE int H3DMath::BitsForInteger( int i ) {
	return ILog2( (float)i ) + 1;
}

ID_INLINE int H3DMath::MaskForFloatSign( float f ) {
	return ( (*reinterpret_cast<int *>(&f)) >> 31 );
}

ID_INLINE int H3DMath::MaskForIntegerSign( int i ) {
	return ( i >> 31 );
}

ID_INLINE int H3DMath::FloorPowerOfTwo( int x ) {
	return CeilPowerOfTwo( x ) >> 1;
}

ID_INLINE int H3DMath::CeilPowerOfTwo( int x ) {
	x--;
	x |= x >> 1;
	x |= x >> 2;
	x |= x >> 4;
...
ID_INLINE bool H3DMath::IsPowerOfTwo( int x ) {
	return ( x & ( x - 1 ) ) == 0 && x > 0;
}

ID_INLINE bool H3DMath::IsPowerOfTwo( unsigned int x ) {
	return ( x & ( x - 1 ) ) == 0;
}

ID_INLINE int H3DMath::BitCount( int x ) {
	x -= ( ( x >> 1 ) & 0x55555555 );
...
ID_INLINE int H3DMath::BitReverse( int x ) {
...
ID_INLINE int H3DMath::Abs( int x ) {
   int y = x >> 31;
   return ( ( x ^ y ) - y );
}

ID_INLINE float H3DMath::Fabs( float f ) {
	int tmp = *reinterpret_cast<int *>( &f );
	tmp &= 0x7FFFFFFF;
	return *reinterpret_cast<float *>( &tmp );
}

ID_INLINE float H3DMath::Floor( float f ) {
	return floorf( f );
}

ID_INLINE float H3DMath::Ceil( float f ) {
	return ceilf( f );
}

ID_INLINE float H3DMath::Rint( float f ) {
	return floorf( f + 0.5f );
}

ID_INLINE int H3DMath::Ftoi( float f ) {
	return (int) f;
}

ID_INLINE int H3DMath::FtoiFast( float f ) {
#ifdef _WIN32
	int i;
	__asm fld		f
...
ID_INLINE unsigned long H3DMath::Ftol( float f ) {
	return (unsigned long) f;
}

ID_INLINE unsigned long H3DMath::FtolFast( float f ) {
#ifdef _WIN32
...
ID_INLINE signed char H3DMath::ClampChar( int i ) {
	if ( i < -128 ) {
		return -128;
	}
	if ( i > 127 ) {
...
ID_INLINE signed short H3DMath::ClampShort( int i ) {
	if ( i < -32768 ) {
		return -32768;
	}
	if ( i > 32767 ) {
...
ID_INLINE int H3DMath::ClampInt( int min, int max, int value ) {
	if ( value < min ) {
		return min;
	}
	if ( value > max ) {
...
ID_INLINE float H3DMath::ClampFloat( float min, float max, float value ) {
	if ( value < min ) {
		return min;
	}
	if ( value > max ) {
...
ID_INLINE float H3DMath::AngleNormalize360( float angle ) {
...
ID_INLINE float H3DMath::AngleNormalize180( float angle ) {
...
ID_INLINE float H3DMath::AngleDelta( float angle1, float angle2 ) {
	return AngleNormalize180( angle1 - angle2 );
}

ID_INLINE int H3DMath::FloatHash( const float *array, const int numFloats ) {
	int i, hash = 0;
	const int *ptr;
