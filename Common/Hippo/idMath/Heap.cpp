...
void idHeap::Init () {
	...
	mediumFirstFreePage	= 0;								// init medium heap manager
	mediumLastFreePage	= 0;
	mediumFirstUsedPage	= 0;
...
void *idHeap::Allocate16( const dword bytes ) {
	...
	if ( !ptr ) {
		...
		if ( !ptr ) {
			//idLib::common->FatalError( "malloc failure for %i", bytes );
		}
	}
...
void idHeap::MediumFree( void *ptr ) {
	...
	if ( prev && prev->freeBlock ) {
		...
		e->prevFree		= 0;				// link to beginning of free list
...
static debugMemory_t *	mem_debugMemory = 0;
...
void Mem_DumpCompressed( const char *fileName, memorySortType_t memSort, int sortCallStack, int numFrames ) {
	...
	debugMemory_t *b;
	allocInfo_t *a, *nexta, *allocInfo = 0, *sortedAllocInfo = 0, *prevSorted, *nextSorted;
	H3DStr module, funcName;
	FILE *f;
	...
	for ( b = mem_debugMemory; b; b = b->next ) {
		...
		if ( numFrames && b->frameNumber < frameNumber - numFrames ) {
			continue;
		}
		...
		for ( a = allocInfo; a; a = a->next ) {
			...
			if ( H3DStr::Cmp( a->fileName, b->fileName ) != 0 ) {
				continue;
			}
			a->numAllocs++;
			a->size += b->size;
	...
	for ( a = allocInfo; a; a = nexta ) {
		...
		switch( memSort ) {
			...
			case MEMSORT_LOCATION: {
				for ( nextSorted = sortedAllocInfo; nextSorted; nextSorted = nextSorted->next ) {
					r = H3DStr::Cmp( Mem_CleanupFileName( a->fileName ), Mem_CleanupFileName( nextSorted->fileName ) );
...
void Mem_DumpCompressed_f( const idCmdArgs &args ) {
	...
	while( arg[0] == '-' ) {
		arg = args.Argv( ++argNum );
		if ( H3DStr::Icmp( arg, "s" ) == 0 ) {
			memSort = MEMSORT_SIZE;
		} else if ( H3DStr::Icmp( arg, "l" ) == 0 ) {
			memSort = MEMSORT_LOCATION;
		} else if ( H3DStr::Icmp( arg, "a" ) == 0 ) {
			memSort = MEMSORT_NUMALLOCS;
		} else if ( H3DStr::Icmp( arg, "cs1" ) == 0 ) {
			memSort = MEMSORT_CALLSTACK;
			sortCallStack = 2;
		} else if ( H3DStr::Icmp( arg, "cs2" ) == 0 ) {
			memSort = MEMSORT_CALLSTACK;
			sortCallStack = 1;
		} else if ( H3DStr::Icmp( arg, "cs3" ) == 0 ) {
			memSort = MEMSORT_CALLSTACK;
			sortCallStack = 0;
		} else if ( arg[0] == 'f' ) {
...
void *Mem_AllocDebugMemory( const int size, const char *fileName, const int lineNumber, const bool align16 ) {
	...
	m->frameNumber = frameNumber;//idLib::frameNumber;
	m->size = size;
	...
	mem_debugMemory = m;
	//idLib::->GetCallStack( m->callStack, MAX_CALLSTACK_DEPTH );
...
*/
void Mem_EnableLeakTest( const char *name ) {
	H3DStr::Copynz( mem_leakName, name, sizeof( mem_leakName ) );
}
#pragma warning(pop)
