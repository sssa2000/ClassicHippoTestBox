...
//===============================================================
//                                                        M
//  SSE implementation of H3DSIMDProcessor                MrE
//                                                        E
//===============================================================
...
ALIGN4_INIT1( float SIMD_SP_PI, H3DMath::PI );
ALIGN4_INIT1( float SIMD_SP_halfPI, H3DMath::HALF_PI );
ALIGN4_INIT1( float SIMD_SP_twoPI, H3DMath::TWO_PI );
ALIGN4_INIT1( float SIMD_SP_oneOverTwoPI, 1.0f / H3DMath::TWO_PI );
ALIGN4_INIT1( float SIMD_SP_infinity, H3DMath::INFINITY );
...
float SSE_SinZeroHalfPI( float a ) {
	...
	assert( a >= 0.0f && a <= H3DMath::HALF_PI );
...
float SSE_Sin( float a ) {
	...
	__asm {
		...
		cmpnltss	xmm2, SIMD_SP_halfPI		// xmm2 = ( fabs( PI - a ) >= H3DMath::HALF_PI ) ? 0xFFFFFFFF : 0x00000000
		movss		xmm3, SIMD_SP_PI			// xmm3 = PI
		xorps		xmm3, xmm1					// xmm3 = PI ^ signbit( PI - a )
		andps		xmm3, xmm2					// xmm3 = ( fabs( PI - a ) >= H3DMath::HALF_PI ) ? ( PI ^ signbit( PI - a ) ) : 0.0f
		andps		xmm2, SIMD_SP_signBitMask	// xmm2 = ( fabs( PI - a ) >= H3DMath::HALF_PI ) ? SIMD_SP_signBitMask : 0.0f
		xorps		xmm0, xmm2
		addps		xmm0, xmm3
	...
	if ( ( a < 0.0f ) || ( a >= H3DMath::TWO_PI ) ) {
		a -= floorf( a / H3DMath::TWO_PI ) * H3DMath::TWO_PI;
	}

	a = H3DMath::PI - a;
	if ( fabs( a ) >= H3DMath::HALF_PI ) {
		a = ( ( a < 0.0f ) ? -H3DMath::PI : H3DMath::PI ) - a;
	}
...
void SSE_Sin4( float a[4], float s[4] ) {
	__asm {
		...
		xorps		xmm2, xmm1					// xmm2 = fabs( PI - a )
		cmpnltps	xmm2, SIMD_SP_halfPI		// xmm2 = ( fabs( PI - a ) >= H3DMath::HALF_PI ) ? 0xFFFFFFFF : 0x00000000
...
============
*/
const char * H3DSIMD_SSE::GetName( void ) const {
	return "MMX & SSE";
}
...
============
*/
void VPCALL H3DSIMD_SSE::Add( float *dst, const float constant, const float *src, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::Add( float *dst, const float *src0, const float *src1, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::Sub( float *dst, const float constant, const float *src, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::Sub( float *dst, const float *src0, const float *src1, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::Mul( float *dst, const float constant, const float *src, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::Mul( float *dst, const float *src0, const float *src1, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::Div( float *dst, const float constant, const float *src, const int count ) {
	int pre, post;
...
============
*/
void VPCALL H3DSIMD_SSE::Div( float *dst, const float *src0, const float *src1, const int count ) {
	int		pre,post;
...
============
*/
void VPCALL H3DSIMD_SSE::MulAdd( float *dst, const float constant, const float *src, const int count ) {
	float c = constant;
	MULADD_FEW( += )
...
============
*/
void VPCALL H3DSIMD_SSE::MulAdd( float *dst, const float *src0, const float *src1, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::MulSub( float *dst, const float constant, const float *src, const int count ) {
	float c = constant;
	MULADD_FEW( -= )
	Simd_MulAdd( dst, -constant, src, count );
}
...
============
*/
void VPCALL H3DSIMD_SSE::MulSub( float *dst, const float *src0, const float *src1, const int count ) {
	for ( int i = 0; i < count; i++ ) {
		dst[i] -= src0[i] + src1[i];
	}
}
...
============
*/
void VPCALL H3DSIMD_SSE::Dot( float *dst, const H3DVec3 &constant, const H3DVec3 *src, const int count ) {
	__asm
	{
		mov			eax, count
		mov			edi, constant
...
============
*/
void VPCALL H3DSIMD_SSE::Dot( float *dst, const H3DVec3 &constant, const H3DPlane *src, const int count ) {
	__asm {
		mov			eax, count
		mov			edi, constant
		mov			edx, eax
...
============
*/
void VPCALL H3DSIMD_SSE::Dot( float *dst, const H3DVec3 &constant, const H3DDrawVert *src, const int count ) {

	assert( sizeof( H3DDrawVert ) == DRAWVERT_SIZE );
	assert( (int)&((H3DDrawVert *)0)->xyz == DRAWVERT_XYZ_OFFSET );
...
============
*/
void VPCALL H3DSIMD_SSE::Dot( float *dst, const H3DPlane &constant, const H3DVec3 *src, const int count ) {
	__asm
	{
		mov			eax, count
		mov			edi, constant
...
============
*/
void VPCALL H3DSIMD_SSE::Dot( float *dst, const H3DPlane &constant, const H3DPlane *src, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::Dot( float *dst, const H3DPlane &constant, const H3DDrawVert *src, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::Dot( float *dst, const H3DVec3 *src0, const H3DVec3 *src1, const int count ) {
	__asm
	{
		mov			eax, count
		mov			edi, src0
...
============
*/
void VPCALL H3DSIMD_SSE::Dot( float &dot, const float *src1, const float *src2, const int count ) {
	switch( count ) {
		case 0:
			dot = 0.0f;
			return;
...
============
*/
void VPCALL H3DSIMD_SSE::CmpGT( byte *dst, const float *src0, const float constant, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::CmpGT( byte *dst, const byte bitNum, const float *src0, const float constant, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::CmpGE( byte *dst, const float *src0, const float constant, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::CmpGE( byte *dst, const byte bitNum, const float *src0, const float constant, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::CmpLT( byte *dst, const float *src0, const float constant, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::CmpLT( byte *dst, const byte bitNum, const float *src0, const float constant, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::CmpLE( byte *dst, const float *src0, const float constant, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::CmpLE( byte *dst, const byte bitNum, const float *src0, const float constant, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::MinMax( float &min, float &max, const float *src, const int count ) {
	int i, pre, post;
...
============
*/
void VPCALL H3DSIMD_SSE::MinMax( H3DVec2 &min, H3DVec2 &max, const H3DVec2 *src, const int count ) {
	__asm {
		mov			eax, count
		test		eax, eax
		movss		xmm0, H3DMath::INFINITY
		xorps		xmm1, xmm1
...
============
*/
void VPCALL H3DSIMD_SSE::MinMax( H3DVec3 &min, H3DVec3 &max, const H3DVec3 *src, const int count ) {
	__asm {
...
============
*/
void VPCALL H3DSIMD_SSE::MinMax( H3DVec3 &min, H3DVec3 &max, const H3DDrawVert *src, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::MinMax( H3DVec3 &min, H3DVec3 &max, const H3DDrawVert *src, const int *indexes, const int count ) {
...
============
*/
void VPCALL H3DSIMD_SSE::Clamp( float *dst, const float *src, const float min, const float max, const int count ) {
	int	i, pre, post;
...
/*
============
H3DSIMD_SSE::ClampMin
============
*/
void VPCALL H3DSIMD_SSE::ClampMin( float *dst, const float *src, const float min, const int count ) {
	int	i, pre, post;
...
/*
============
H3DSIMD_SSE::ClampMax
============
*/
void VPCALL H3DSIMD_SSE::ClampMax( float *dst, const float *src, const float max, const int count ) {
	int	i, pre, post;
...
============
*/
void VPCALL H3DSIMD_SSE::Zero16( float *dst, const int count ) {
	__asm {
		mov		edx, dst
		mov		eax, count
		add		eax, 3
...
/*
============
H3DSIMD_SSE::Negate16
============
*/
void VPCALL H3DSIMD_SSE::Negate16( float *dst, const int count ) {
	__asm {
		mov		edx, dst
		mov		eax, count
		add		eax, 3
...
============
*/
void VPCALL H3DSIMD_SSE::Copy16( float *dst, const float *src, const int count ) {
	__asm {
		mov		ecx, src
		mov		edx, dst
		mov		eax, count
...
============
*/
void VPCALL H3DSIMD_SSE::Add16( float *dst, const float *src1, const float *src2, const int count ) {
	__asm {
		mov		ecx, src1
		mov		edx, src2
		mov		esi, dst
...
============
*/
void VPCALL H3DSIMD_SSE::Sub16( float *dst, const float *src1, const float *src2, const int count ) {
	__asm {
		mov		ecx, src1
		mov		edx, src2
		mov		esi, dst
...
============
*/
void VPCALL H3DSIMD_SSE::Mul16( float *dst, const float *src1, const float constant, const int count ) {
	__asm {
		mov		ecx, dst
		mov		edx, src1
		mov		eax, count
...
/*
============
H3DSIMD_SSE::AddAssign16
============
*/
void VPCALL H3DSIMD_SSE::AddAssign16( float *dst, const float *src, const int count ) {
	__asm {
		mov		ecx, dst
		mov		edx, src
		mov		eax, count
...
/*
============
H3DSIMD_SSE::SubAssign16
============
*/
void VPCALL H3DSIMD_SSE::SubAssign16( float *dst, const float *src, const int count ) {
	__asm {
		mov		ecx, dst
		mov		edx, src
		mov		eax, count
...
/*
============
H3DSIMD_SSE::MulAssign16
============
*/
void VPCALL H3DSIMD_SSE::MulAssign16( float *dst, const float constant, const int count ) {
	__asm {
		mov		ecx, dst
		mov		eax, count
		add		eax, 3
...
============
*/
void VPCALL H3DSIMD_SSE::MatX_MultiplyVecX( H3DVecX &dst, const H3DMatX &mat, const H3DVecX &vec ) {
...
/*
============
H3DSIMD_SSE::MatX_MultiplyAddVecX
...
============
*/
void VPCALL H3DSIMD_SSE::MatX_MultiplyAddVecX( H3DVecX &dst, const H3DMatX &mat, const H3DVecX &vec ) {
#define STORE1( offset, reg1, reg2 )		\
...
/*
============
H3DSIMD_SSE::MatX_MultiplySubVecX
...
============
*/
void VPCALL H3DSIMD_SSE::MatX_MultiplySubVecX( H3DVecX &dst, const H3DMatX &mat, const H3DVecX &vec ) {
#define STORE1( offset, reg1, reg2 )		\
	__asm movss		reg2, [eax+offset]		\
...
/*
============
H3DSIMD_SSE::MatX_TransposeMultiplyVecX
...
============
*/
void VPCALL H3DSIMD_SSE::MatX_TransposeMultiplyVecX( H3DVecX &dst, const H3DMatX &mat, const H3DVecX &vec ) {
#define STORE1( offset, reg1, reg2 )		\
	__asm movss		[eax+offset], reg1
#define STORE2LO( offset, reg1, reg2 )		\
	__asm movlps	[eax+offset], reg1
...
/*
============
H3DSIMD_SSE::MatX_TransposeMultiplyAddVecX
...
============
*/
void VPCALL H3DSIMD_SSE::MatX_TransposeMultiplyAddVecX( H3DVecX &dst, const H3DMatX &mat, const H3DVecX &vec ) {
#define STORE1( offset, reg1, reg2 )		\
	__asm movss		reg2, [eax+offset]		\
	__asm addss		reg2, reg1				\
	__asm movss		[eax+offset], reg2
...
/*
============
void H3DSIMD_SSE::MatX_TransposeMultiplySubVecX
...
============
*/
void VPCALL H3DSIMD_SSE::MatX_TransposeMultiplySubVecX( H3DVecX &dst, const H3DMatX &mat, const H3DVecX &vec ) {
#define STORE1( offset, reg1, reg2 )		\
	__asm movss		reg2, [eax+offset]		\
	__asm subss		reg2, reg1				\
	__asm movss		[eax+offset], reg2
...
============
*/
void VPCALL H3DSIMD_SSE::MatX_MultiplyMatX( H3DMatX &dst, const H3DMatX &m1, const H3DMatX &m2 ) {
	int i, j, k, l, n;
	float *dstPtr;
...
/*
============
H3DSIMD_SSE::MatX_TransposeMultiplyMatX
...
============
*/
void VPCALL H3DSIMD_SSE::MatX_TransposeMultiplyMatX( H3DMatX &dst, const H3DMatX &m1, const H3DMatX &m2 ) {
	int i, j, k, l, n;
	float *dstPtr;
	const float *m1Ptr, *m2Ptr;
	double sum;
...
/*
============
H3DSIMD_SSE::MatX_LowerTriangularSolve
...
============
*/
void VPCALL H3DSIMD_SSE::MatX_LowerTriangularSolve( const H3DMatX &L, float *x, const float *b, const int n, int skip ) {
	int nc;
	const float *lptr;
...
/*
============
H3DSIMD_SSE::MatX_LowerTriangularSolveTranspose
...
============
*/
void VPCALL H3DSIMD_SSE::MatX_LowerTriangularSolveTranspose( const H3DMatX &L, float *x, const float *b, const int n ) {
	int nc;
	const float *lptr;
...
============
*/
bool VPCALL H3DSIMD_SSE::MatX_LDLTFactor( H3DMatX &mat, H3DVecX &invDiag, const int n ) {
#if 1
...
/*
============
H3DSIMD_SSE::BlendJoints
============
*/
#define REFINE_BLENDJOINTS_RECIPROCAL

void VPCALL H3DSIMD_SSE::BlendJoints( H3DJointQuat *joints, const H3DJointQuat *blendJoints, const float lerp, const int *index, const int numJoints ) {
	int i;
	...
	for ( i = 0; i <= numJoints - 4; i += 4 ) {
		...
		sinom[0] = H3DMath::RSqrt( scale0[0] );
		sinom[1] = H3DMath::RSqrt( scale0[1] );
		sinom[2] = H3DMath::RSqrt( scale0[2] );
		sinom[3] = H3DMath::RSqrt( scale0[3] );
	...
	for ( ; i < numJoints; i++ ) {
		...
		H3DVec3 &jointVert = joints[n].t;
		const H3DVec3 &blendVert = blendJoints[n].t;

		jointVert[0] += lerp * ( blendVert[0] - jointVert[0] );
		jointVert[1] += lerp * ( blendVert[1] - jointVert[1] );
		jointVert[2] += lerp * ( blendVert[2] - jointVert[2] );

		H3DQuat &jointQuat = joints[n].q;
		const H3DQuat &blendQuat = blendJoints[n].q;
		...
		scale0 = H3DMath::Sin16( ( 1.0f - lerp ) * omega ) * sinom;
		scale1 = H3DMath::Sin16( lerp * omega ) * sinom;
...
/*
============
H3DSIMD_SSE::ConvertJointQuatsToJointMats
============
*/
void VPCALL H3DSIMD_SSE::ConvertJointQuatsToJointMats( H3DJointMat *jointMats, const H3DJointQuat *jointQuats, const int numJoints ) {

	assert( sizeof( H3DJointQuat ) == JOINTQUAT_SIZE );
	assert( sizeof( H3DJointMat ) == JOINTMAT_SIZE );
	assert( (int)(&((H3DJointQuat *)0)->t) == (int)(&((H3DJointQuat *)0)->q) + (int)sizeof( ((H3DJointQuat *)0)->q ) );
...
/*
============
H3DSIMD_SSE::ConvertJointMatsToJointQuats
============
*/
void VPCALL H3DSIMD_SSE::ConvertJointMatsToJointQuats( H3DJointQuat *jointQuats, const H3DJointMat *jointMats, const int numJoints ) {
	...
	for ( int i = 0; i < numJoints; i++ ) {
		...
		float s = H3DMath::InvSqrt( t ) * 0.5f;
...
/*
============
H3DSIMD_SSE::TransformJoints
============
*/
void VPCALL H3DSIMD_SSE::TransformJoints( H3DJointMat *jointMats, const int *parents, const int firstJoint, const int lastJoint ) {
#if 1
...
/*
============
H3DSIMD_SSE::UntransformJoints
============
*/
void VPCALL H3DSIMD_SSE::UntransformJoints( H3DJointMat *jointMats, const int *parents, const int firstJoint, const int lastJoint ) {
#if 1
...
/*
============
H3DSIMD_SSE::TransformVerts
============
*/
void VPCALL H3DSIMD_SSE::TransformVerts( H3DDrawVert *verts, const int numVerts, const H3DJointMat *joints, const H3DVec4 *weights, const int *index, const int numWeights ) {
#if 1

	assert( sizeof( H3DDrawVert ) == DRAWVERT_SIZE );
	assert( (int)&((H3DDrawVert *)0)->xyz == DRAWVERT_XYZ_OFFSET );
	assert( sizeof( H3DVec4 ) == JOINTWEIGHT_SIZE );
	assert( sizeof( H3DJointMat ) == JOINTMAT_SIZE );
...
/*
============
H3DSIMD_SSE::TracePointCull
============
*/
void VPCALL H3DSIMD_SSE::TracePointCull( byte *cullBits, byte &totalOr, const float radius, const H3DPlane *planes, const H3DDrawVert *verts, const int numVerts ) {
#if 1
...
/*
============
H3DSIMD_SSE::DecalPointCull
============
*/
void VPCALL H3DSIMD_SSE::DecalPointCull( byte *cullBits, const H3DPlane *planes, const H3DDrawVert *verts, const int numVerts ) {
#if 1
	...
	for ( i = 0; i < numVerts; i += 2 ) {
		...
		const H3DVec3 &v0 = verts[i+0].xyz;
		const H3DVec3 &v1 = verts[i+1].xyz;
	...
	if ( numVerts & 1 ) {
		...
		const H3DVec3 &v = verts[numVerts - 1].xyz;
...
/*
============
H3DSIMD_SSE::OverlayPointCull
============
*/
void VPCALL H3DSIMD_SSE::OverlayPointCull( byte *cullBits, H3DVec2 *texCoords, const H3DPlane *planes, const H3DDrawVert *verts, const int numVerts ) {
#if 1
	...
	const H3DPlane &p0 = planes[0];
	const H3DPlane &p1 = planes[1];
	...
	if ( numVerts & 1 ) {
		...
		const H3DPlane &p0 = planes[0];
		const H3DPlane &p1 = planes[1];
		const H3DVec3 &v0 = verts[numVerts - 1].xyz;
...
/*
============
H3DSIMD_SSE::DeriveTriPlanes
============
*/
void VPCALL H3DSIMD_SSE::DeriveTriPlanes( H3DPlane *planes, const H3DDrawVert *verts, const int numVerts, const int *indexes, const int numIndexes ) {
#if 1
	...
	for ( i = 0; i <= numIndexes - 12; i += 12 ) {
		...
		tmp[0] = H3DMath::RSqrt( tmp[0] );
		tmp[1] = H3DMath::RSqrt( tmp[1] );
		tmp[2] = H3DMath::RSqrt( tmp[2] );
		tmp[3] = H3DMath::RSqrt( tmp[3] );
	...
	for ( ; i < numIndexes; i += 3 ) {
		...
		tmp = H3DMath::RSqrt( n0 * n0 + n1 * n1 + n2 * n2 );
...
/*
============
H3DSIMD_SSE::DeriveTangents
============
*/
//#define REFINE_TANGENT_SQUAREROOT
#define FIX_DEGENERATE_TANGENT

void VPCALL H3DSIMD_SSE::DeriveTangents( H3DPlane *planes, H3DDrawVert *verts, const int numVerts, const int *indexes, const int numIndexes ) {
	int i;

	assert( sizeof( H3DDrawVert ) == DRAWVERT_SIZE );
	assert( (int)&((H3DDrawVert *)0)->normal == DRAWVERT_NORMAL_OFFSET );
	assert( (int)&((H3DDrawVert *)0)->tangents[0] == DRAWVERT_TANGENT0_OFFSET );
	assert( (int)&((H3DDrawVert *)0)->tangents[1] == DRAWVERT_TANGENT1_OFFSET );
	...
	for ( ; i < numIndexes; i += 3 ) {
		...
		tmp = H3DMath::RSqrt( t0 * t0 + t1 * t1 + t2 * t2 );
		*(unsigned long *)&tmp ^= signBit[0];
		...
		tmp = H3DMath::RSqrt( t3 * t3 + t4 * t4 + t5 * t5 );
		*(unsigned long *)&tmp ^= signBit[0];
...
/*
============
H3DSIMD_SSE::DeriveUnsmoothedTangents
============
*/
#define DERIVE_UNSMOOTHED_BITANGENT

void VPCALL H3DSIMD_SSE::DeriveUnsmoothedTangents( H3DDrawVert *verts, const dominantTri_s *dominantTris, const int numVerts ) {
	int i, j;
...
/*
============
H3DSIMD_SSE::NormalizeTangents
============
*/
void VPCALL H3DSIMD_SSE::NormalizeTangents( H3DDrawVert *verts, const int numVerts ) {
	...
	loopVert4:
		...
		// normalize 4 H3DDrawVert::normal
		...
		// save the 4 H3DDrawVert::normal to project the tangents
		...
		// project and normalize 4 H3DDrawVert::tangent[0]
		...
		// project and normalize 4 H3DDrawVert::tangent[1]
	...
	loopVert1:
		...
		// normalize one H3DDrawVert::normal
		...
		// project and normalize one H3DDrawVert::tangent[0]
		...
		// project and normalize one H3DDrawVert::tangent[1]
...
/*
============
H3DSIMD_SSE::CreateTextureSpaceLightVectors
============
*/
void VPCALL H3DSIMD_SSE::CreateTextureSpaceLightVectors( H3DVec3 *lightVectors, const H3DVec3 &lightOrigin, const H3DDrawVert *verts, const int numVerts, const int *indexes, const int numIndexes ) {
	...
	H3DVec3 localLightOrigin = lightOrigin;
...
/*
============
H3DSIMD_SSE::CreateSpecularTextureCoords
============
*/
void VPCALL H3DSIMD_SSE::CreateSpecularTextureCoords( H3DVec4 *texCoords, const H3DVec3 &lightOrigin, const H3DVec3 &viewOrigin, const H3DDrawVert *verts, const int numVerts, const int *indexes, const int numIndexes ) {
	...
	for ( int i = 0; i < numVerts; i++ ) {
		...
		ilength = H3DMath::RSqrt( lightDir[0] * lightDir[0] + lightDir[1] * lightDir[1] + lightDir[2] * lightDir[2] );
		lightDir[0] *= ilength;
		lightDir[1] *= ilength;
		lightDir[2] *= ilength;

		ilength = H3DMath::RSqrt( viewDir[0] * viewDir[0] + viewDir[1] * viewDir[1] + viewDir[2] * viewDir[2] );
		viewDir[0] *= ilength;
		viewDir[1] *= ilength;
		viewDir[2] *= ilength;
	...
	ALIGN16( float tangent5[4]; )
	H3DVec3 localLightOrigin = lightOrigin;
	H3DVec3 localViewOrigin = viewOrigin;
	...
	for ( int i = 0; i < numVerts; i++ ) {
		...
		temp[0] = H3DMath::RSqrt( temp[0] );
		temp[1] = H3DMath::RSqrt( temp[1] );
		temp[2] = H3DMath::RSqrt( temp[2] );
		temp[3] = H3DMath::RSqrt( temp[3] );
...
/*
============
H3DSIMD_SSE::CreateShadowCache
============
*/
int VPCALL H3DSIMD_SSE::CreateShadowCache( H3DVec4 *vertexCache, int *vertRemap, const H3DVec3 &lightOrigin, const H3DDrawVert *verts, const int numVerts ) {
#if 1
	int outVerts;
...
/*
============
H3DSIMD_SSE::CreateVertexProgramShadowCache
============
*/
int VPCALL H3DSIMD_SSE::CreateVertexProgramShadowCache( H3DVec4 *vertexCache, const H3DDrawVert *verts, const int numVerts ) {
#if 1
...
/*
============
H3DSIMD_SSE::UpSamplePCMTo44kHz

  Duplicate samples for 44kHz output.
============
*/
void H3DSIMD_SSE::UpSamplePCMTo44kHz( float *dest, const short *src, const int numSamples, const int kHz, const int numChannels ) {
	if ( kHz == 11025 ) {
		if ( numChannels == 1 ) {
...
/*
============
H3DSIMD_SSE::UpSampleOGGTo44kHz

  Duplicate samples for 44kHz output.
============
*/
void H3DSIMD_SSE::UpSampleOGGTo44kHz( float *dest, const float * const *ogg, const int numSamples, const int kHz, const int numChannels ) {
	if ( kHz == 11025 ) {
		if ( numChannels == 1 ) {
...
/*
============
H3DSIMD_SSE::MixSoundTwoSpeakerMono
============
*/
void VPCALL H3DSIMD_SSE::MixSoundTwoSpeakerMono( float *mixBuffer, const float *samples, const int numSamples, const float lastV[2], const float currentV[2] ) {
#if 1
...
/*
============
H3DSIMD_SSE::MixSoundTwoSpeakerStereo
============
*/
void VPCALL H3DSIMD_SSE::MixSoundTwoSpeakerStereo( float *mixBuffer, const float *samples, const int numSamples, const float lastV[2], const float currentV[2] ) {
#if 1
...
/*
============
H3DSIMD_SSE::MixSoundSixSpeakerMono
============
*/
void VPCALL H3DSIMD_SSE::MixSoundSixSpeakerMono( float *mixBuffer, const float *samples, const int numSamples, const float lastV[6], const float currentV[6] ) {
#if 1
...
/*
============
H3DSIMD_SSE::MixSoundSixSpeakerStereo
============
*/
void VPCALL H3DSIMD_SSE::MixSoundSixSpeakerStereo( float *mixBuffer, const float *samples, const int numSamples, const float lastV[6], const float currentV[6] ) {
#if 1
...
/*
============
H3DSIMD_SSE::MixedSoundToSamples
============
*/
void VPCALL H3DSIMD_SSE::MixedSoundToSamples( short *samples, const float *mixBuffer, const int numSamples ) {
#if 1
